...
export const PokerTable: React.FC<PokerTableProps> = ({
  ...
  const gameManager = useMemo(() => {
    if (!tableId) {
      logger.error('[PokerTable] Cannot initialize GameManager: tableId is undefined');
      return null;
    }
    return new GameManager(tableId);
  }, [tableId]);
  ...
  useEffect(() => {
    if (user && currentPlayerId && user.displayName && tableId) {
      gameManager?.refreshPlayerUsername(currentPlayerId, user.displayName);
    }
  }, [user, currentPlayerId, tableId, gameManager]);
  ...
  const handleAction = async (action: 'fold' | 'check' | 'call' | 'raise', amount?: number) => {
    ...
    if (!onPlayerAction || !tableData || !currentPlayerId) {
      logger.log('[PokerTable] handleAction aborted:', { onPlayerAction: !!onPlayerAction, tableData: !!tableData, currentPlayerId });
      return;
    }

    const player = tableData.players.find(p => p.id === currentPlayerId);
    if (!player) return;
    ...
    switch (action) {
      case 'fold':
        playerUpdates.hasFolded = true;
        break;
      case 'check':
        updates.roundBets = {
          ...tableData.roundBets,
          [currentPlayerId]: tableData.roundBets[currentPlayerId] || 0
        };
        break;
      case 'call':
        const callAmount = tableData.currentBet - (tableData.roundBets[currentPlayerId] || 0);
        playerUpdates.chips = player.chips - callAmount;
        updates.pot = tableData.pot + callAmount;
        updates.roundBets = {
          ...tableData.roundBets,
          [currentPlayerId]: tableData.currentBet
        };
        break;
      case 'raise':
        const raiseBet = validAmount! - (tableData.roundBets[currentPlayerId] || 0);
        playerUpdates.chips = player.chips - raiseBet;
        updates = {
          pot: tableData.pot + raiseBet,
          roundBets: {
            ...tableData.roundBets,
            [currentPlayerId]: validAmount!
          },
          currentBet: validAmount!,
          minRaise: validAmount! * 2,
          lastBettor: currentPlayerId
        };
        break;
    }

    updates.lastAction = action;
    updates.lastActivePlayer = currentPlayerId;
    
    if (action === 'check' && tableData.currentBet === 0) {
      updates.currentPlayerIndex = (tableData.currentPlayerIndex + 1) % tableData.players.length;
    }

    // Update player state if needed
    if (Object.keys(playerUpdates).length > 0) {
      await updatePlayerState(currentPlayerId, playerUpdates);
    }

    // Update table state
    if (Object.keys(updates).length > 0) {
      await updateTable(updates);
    }
  ...
  if (validationError.length > 0 || tableError) {
    return (
      <div className="p-4 bg-red-100 text-red-800 rounded-md">
        {validationError.join(', ') || tableError || 'An error occurred. Please try refreshing the page.'}
      </div>
    );
  }
  ...
  if (!tableData) {
    return (
      <div className="p-4 bg-red-100 text-red-800 rounded-md">
        Table data not found. Please try refreshing the page.
      </div>
    );
  }
  ...
  const pot = tableData.pot;
  const currentBet = tableData.currentBet;
  const dealerPosition = tableData.dealerPosition;
  const phase = tableData.phase;
  const communityCards = tableData.communityCards;
  const isHandInProgress = tableData.isHandInProgress;
  const activePlayerIndex = tableData.currentPlayerIndex;
  ...
  const playerBet = currentPlayerId && tableData.roundBets ? (tableData.roundBets[currentPlayerId] || 0) : 0;
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-blue-900 to-blue-950 rounded-[40%] shadow-2xl border-4 border-blue-800">
          <div className="absolute inset-[4px] bg-gradient-to-b from-teal-800 to-teal-900 rounded-[39%]">
            ...
            <div className="absolute top-[30%] sm:top-[40%] md:top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 scale-[0.7] sm:scale-[0.8] md:scale-[0.9] lg:scale-100">
              {phase === 'showdown' && tableData.winners && tableData.winners.length > 0 && tableData.winningAmount ? (
                <WinnerDisplay 
                  winnerNames={tableData.winners.map(winnerId => {
                    const player = players.find(p => p.id === winnerId);
                    return player?.name || 'Unknown Player';
                  })}
                  winningAmount={tableData.winningAmount}
                  winningHands={tableData.winningHands}
                />
              ) : (
                ['flop', 'turn', 'river'].includes(phase.toLowerCase()) && (
                  <CommunityCards cards={tableData.communityCards} phase={phase} />
                )
              )}
            </div>
      ...
      {currentPlayerId && currentPlayer && !currentPlayer.hasFolded && (
        <div className="mt-4 sm:mt-6 md:mt-8 flex flex-col items-center gap-3 p-3 bg-gradient-to-b from-gray-800 to-gray-900 rounded-xl shadow-2xl border border-gray-700">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 w-full">
            ...
            <input
              type="number"
              min={tableData.minRaise}
              max={currentPlayer.chips}
              value={raiseAmount}
              onChange={(e) => setRaiseAmount(Math.max(tableData.minRaise, Math.min(currentPlayer.chips, parseInt(e.target.value) || 0)))}
              disabled={!isPlayerTurn}
            ...
            <button
              ...
              disabled={!isPlayerTurn || raiseAmount < tableData.minRaise}
              className={`px-3 py-2 rounded-md text-sm font-semibold ${isPlayerTurn && raiseAmount >= tableData.minRaise ? 'bg-yellow-500 hover:bg-yellow-600 text-gray-900' : 'bg-gray-600 text-gray-300 cursor-not-allowed'}`}
            >
              Raise
            </button>
          </div>
          ...
          {isPlayerTurn && (
            <div className="flex justify-center w-full">
              <TurnTimer table={tableData} isCurrentPlayer={true} />
            </div>
          )}
        </div>
      )}
